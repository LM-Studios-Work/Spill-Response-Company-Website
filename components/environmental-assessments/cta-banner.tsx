export function AssessmentCtaBanner() {
  return (
    <section className="bg-primary py-12 px-4 border-t border-green-800">
      <div className="max-w-4xl mx-auto text-center">
        <div className="inline-flex items-center justify-center p-3 bg-white/10 rounded-full mb-4">
          <span className="material-icons text-accent text-3xl">
            verified_user
          </span>
        </div>
        <h2 className="text-2xl md:text-3xl font-medium text-white mb-2">
          Don&apos;t let paperwork be your downfall.
        </h2>
        <p className="text-green-100 text-lg mb-8">
          Ensure your environmental reporting is accurate and compliance.
        </p>
        <div className="inline-block bg-white/10 backdrop-blur-sm border border-white/20 rounded-lg px-8 py-4">
          <span className="text-green-100 font-medium mr-2">
            Contact SSSK for Advisory Services:
          </span>
          <a
            href="tel:0717881312"
            className="text-accent font-black text-xl md:text-2xl block md:inline mt-2 md:mt-0 hover:text-yellow-300 transition"
          >
            071 788 1312
          </a>
        </div>
      </div>
    </section>
  );
}
